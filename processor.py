...
def anonymize_video(uid, tasks):
    ...
    temp_video = f"tmp/{uid}_blur.mp4"
    ...
    while True:
        ...
        if results and results[0].boxes is not None:
            for box in results[0].boxes:
                ...
                hy1 = max(0, y1)
                hy2 = min(H, y1 + head_h)
                min_kernel, max_kernel = 15, 41
                min_sigma, max_sigma = 6, 20
                ratio = person_h / H
                ...
                if roi.size > 0:
                    roi = cv2.GaussianBlur(roi, (kernel_size, kernel_size), sigma)
                    frame[hy1:hy2, hx1:hx2] = roi

        out.write(frame)
        frame_idx += 1
        tasks[uid]["progress"] = int((frame_idx / total_frames) * 100)
